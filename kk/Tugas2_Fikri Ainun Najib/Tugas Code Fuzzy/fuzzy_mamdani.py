        ...
        for i in range(len(pendapatan)):
            for x in range(len(ipk)):
                    if(ipk[x][0] ==0 and pendapatan[i][0] ==0):
                        aturan.append([0,min(ipk[x][1],pendapatan[i][1])])
                    if(ipk[x][0] ==1 and pendapatan[i][0] ==1):
                        aturan.append([1,min(ipk[x][1],pendapatan[i][1])])
                    if(ipk[x][0] ==2 and pendapatan[i][0] ==1):
                        aturan.append([2,min(ipk[x][1],pendapatan[i][1])])
                    if(ipk[x][0] ==2 and pendapatan[i][0] ==0):
                        aturan.append([3,min(ipk[x][1],pendapatan[i][1])])
    else:
        for i in range(len(ipk)):
            for x in range(len(pendapatan)):
                    if(ipk[i][0] ==0 and pendapatan[x][0] ==0):
                        aturan.append([0,min(ipk[i][1],pendapatan[x][1])])
                    if(ipk[i][0] ==1 and pendapatan[x][0] ==1):
                        aturan.append([1,min(ipk[i][1],pendapatan[x][1])])
                    if(ipk[i][0] ==2 and pendapatan[x][0] ==1):
                        aturan.append([2,min(ipk[i][1],pendapatan[x][1])])
                    if(ipk[i][0] ==2 and pendapatan[x][0] ==0):
                        aturan.append([3,min(ipk[i][1],pendapatan[x][1])])
    return aturan



# aggregasi dan defuzzifikasi
def defuzzifikasi(aturan):
    variabel_beasiswa,i,himpunan,himpunan_output,list_aturan= 3.5,0,[],[],[] # karena ingin mencetak angka 3 maka harus di lebihkan
    for m in range(2):
        if(m == 0):
            print("\n Himpunan Beasiswa Sedikit")
        else:
            print("\n Himpunan Beasiswa Banyak")
        alfa_beasiswa = int(input("Masukan nilai alfa beasiswa : "))
        beta_beasiswa = int(input("Masukan nilai beta beasiswa : "))
        gamma_beasiswa = int(input("Masukan nilai gamma beasiswa : "))
        delta_beasiswa = int(input("Masukan nilai delta beasiswa : "))
        himpunan_output.append([alfa_beasiswa,beta_beasiswa,gamma_beasiswa,delta_beasiswa])
    for p in range(len(aturan)):
        list_aturan.append(aturan[p][1])


    while(i<variabel_beasiswa):
        gamma,alfa = himpunan_output[0][2],himpunan_output[1][0]
        delta,beta = himpunan_output[0][3],himpunan_output[1][1]
        if(i <= gamma):
            himpunan.append([i*list_aturan[0],list_aturan[0]])
        elif(i>gamma and i<delta):
            t = (i-gamma)/(delta-gamma)
            himpunan.append([i*t,t])
        else:
            himpunan.append([i*list_aturan[len(list_aturan)-1],list_aturan[len(list_aturan)-1]])
    
        i+=0.5
    
    pembilang,penyebut = 0,0 
    for x in range(len(himpunan)):
        pembilang += himpunan[x][0]
        penyebut += himpunan[x][1]
